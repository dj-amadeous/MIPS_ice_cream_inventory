...
//hbox price
  ...
  while(!valid_data){
    ...
    _toppings[topping_index].set_price(price);
    _toppings[topping_index].set_cost(cost);
    _toppings[topping_index].set_description(description);
    _toppings[topping_index].set_name(name);
    dialog.close();
  }
}
