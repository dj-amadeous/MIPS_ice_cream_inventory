...
void Mainwin::on_edit_container_click()
{
  ...
  container_index = select_container();
...
//hbox price
  ...
  while(!valid_data){
    ...
    _containers[container_index].set_price(price);
    _containers[container_index].set_cost(cost);
    _containers[container_index].set_description(description);
    _containers[container_index].set_name(name);
    dialog.close();
  }
}
